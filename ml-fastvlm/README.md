...
FastVLM now supports **Universal Hardware Scaling**. The system will automatically detect and use your GPU (NVIDIA or Apple Silicon) if available, falling back to CPU if not.
...
pip install -e .
pip install opencv-python numpy==1.26.4 tavily-python python-dotenv
```
> [!IMPORTANT]
> **Windows Users**: You MUST use `numpy==1.26.4`. Version 2.x will cause the AI system to crash.

### 2. API Key Configuration
Create a file named `.env` in the root directory and add your search key (this allows the AI to search the web for facts it doesn't know):
```env
TAVILY_API_KEY=your_key_here
```

### 3. Running the System
The system is now smart enough to detect your hardware (CUDA/MPS/CPU) automatically.

| Mode | Command | Description |
| :--- | :--- | :--- |
| **Live Vision** | `python main.py` | Uses camera to answer questions written on paper. |
| **Single Image** | `python predict.py --image-file test.jpg` | Analyzes a specific photo. |
| **Custom Prompt** | `python predict.py --image-file test.jpg --prompt "What is this?"` | Ask specific questions about an image. |
...
### Iris OS on iPhone (Mac-free CI/CD)
This fork ships an enhanced version of the Apple SwiftUI demo that adds:
- Sentinel-gated capture (sharpness + motion gating) ported from `sentinel.py`.
- JSON-guided prompting + deterministic tool router (calculator / matrix / web search) ported from `inference.py` and `tools.py`.
- On-device speech input via `SFSpeechRecognizer`.
- A perf HUD (long-press the camera preview) and a 1.5B INT8 default for fast TTFT.

Build it from a Windows machine using GitHub Actions + fastlane → TestFlight by following [`IOS_DEPLOYMENT_RUNBOOK.md`](../IOS_DEPLOYMENT_RUNBOOK.md). No Mac required.
